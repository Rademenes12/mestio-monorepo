    ...
    <div className="max-w-7xl mx-auto space-y-5 animate-pulse">
      <div className="space-y-3">
        ...
        <div className="flex flex-col gap-[9px]">
          {[0, 1].map((i) => (
            <div key={i} className="h-[62px] bg-white rounded-2xl border border-[#E9EEF5]" />
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-[12px] mt-[22px]">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="h-[74px] bg-white rounded-2xl border border-[#E9EEF5]" />
        ))}
      </div>

      <div className="h-[180px] bg-white rounded-[12px] border border-[#E9EEF5]" />
