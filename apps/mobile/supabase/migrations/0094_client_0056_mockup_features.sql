...
CREATE TABLE IF NOT EXISTS public.fixflow_resident_spaces (
  ...
  resident_id uuid NOT NULL REFERENCES public.fixflow_resident_profiles(id) ON DELETE CASCADE,
...
CREATE POLICY "resident_own_spaces" ON public.fixflow_resident_spaces
  ...
  USING (
    EXISTS (
      ...
      WHERE rp.id = fixflow_resident_spaces.resident_id
...
CREATE INDEX IF NOT EXISTS idx_resident_spaces_resident ON public.fixflow_resident_spaces(resident_id);
...
CREATE INDEX IF NOT EXISTS idx_contact_notes_resident ON public.fixflow_contact_notes(resident_id);
...
-- (id, resolution_id, user_id, choice, created_at) — NIE resident_id/vote/
