...
-- max_uses, current_uses) that were never ADDed to the table.
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'fixflow_invitation_codes' AND column_name = 'current_uses'
  ) THEN
    ALTER TABLE public.fixflow_invitation_codes ADD COLUMN current_uses integer DEFAULT 0;
  END IF;
