...
-- Add FK constraint if reports.id is text
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_internal_notes_report' 
    AND table_name = 'fixflow_report_internal_notes'
  ) THEN
    ALTER TABLE public.fixflow_report_internal_notes 
      ADD CONSTRAINT fk_internal_notes_report 
      FOREIGN KEY (report_id) REFERENCES public.fixflow_reports(id) ON DELETE CASCADE;
  END IF;
